...
function startOnePage(myInput){
	...
	*/
	...
	$(".province").bind("load resize", init);
	

	// animate scrolling effect
	var now, end;
	function animateScr(moveTo, duration, distance,heights){
		var top;
		duration = duration || speed;
		switch(moveTo){
			case "down":
				top = "-=" + ( heights * distance ) + "px";
				num += distance;
				break;
			case "up":
				top = "+=" + ( heights * distance ) + "px";
...
//		获取对应的高度
		var texts=$(this).text();
		var heights=0;
		$("."+texts+"d").each(function(indexs,el){
			heights=$(el).height()+heights;
		});
		console.log(heights)
		if(num < to){
			animateScr("down", speed, dif,heights);
		}else if(num > to){
			animateScr("up", speed, dif,heights);
		}
	});
